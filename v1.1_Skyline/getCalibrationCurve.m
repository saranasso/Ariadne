...
% USA		
% % % % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%		
function [regression]=getCalibrationCurve(ratio,concentration)
% it checks for linear relationship and fit the line

% check whether there is any linear relationship
[r p] = corr(ratio,concentration,'tail','gt','rows','pairwise');
...
if p<0.05 & r^2>0.9 % if there is linear relationship
    ...
    axis([0,max(ratio),0,max(concentration)])
    axis manual
    axis(axis)
    hold on
    plot(ratio,concentration,'o')
    titolo=[{'Calibration Curve'}];
    title(titolo)
    x_SD=sqrt(ratio);
    y_SD=sqrt(concentration);
    
    [m,m_SE,q,q_SE,min_quad_distXY,residual,covar_post,J,SE]=fitlineXY(ratio,x_SD,concentration,y_SD,1,0);
    hold on
    x=(0:0.1:max(ratio));
    plot(x,m*x+q,'r')
