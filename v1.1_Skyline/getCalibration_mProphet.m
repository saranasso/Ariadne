...
mProphet_methods={'Skyline_mProphet'}; %TODO code integration
standard_ratios=[];
k=1;
        ...
        for numMethod=1:numel(mProphet_methods)
            if isfield(pepLibrarySRM(i).ratio_mProphet, mProphet_methods{numMethod})
                ...
                if strcmp(settings.constantTarget, 'light')
                    eval(['standard_ratios.' mProphet_methods{numMethod}  '(k,1)=nanmedian([pepLibrarySRM(i).ratio_mProphet(molarities_files(idxs_cc_dataFiles,2)).' mProphet_methods{numMethod} ' ].^(-1));']); % heavy to light  here cause light is constant and unknown while heavy is variable and known
