    ...
    if pepLibrarySRM(i).hasRef
        for j = 1:numel(pepLibrarySRM(i).ratio)
            if ~isnan(pepLibrarySRM(i).ratio(j))
                ...
                quants=cat(1,quants,{pepLibrarySRM(i).protein,pepLibrarySRM(i).sequence,...
                    pepLibrarySRM(i).charge,pepLibrarySRM(i).estRetTime(j),...
                    pepLibrarySRM(i).dataFile{j},pepLibrarySRM(i).ratio(j),currentAbsQuant});
            else
                continue
            end
        end
