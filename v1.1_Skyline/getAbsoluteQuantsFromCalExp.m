                    ...
                    for numMethod=1:numel(mProphet_methods)
                        if isfield(pepLibrarySRMCal(idx_lib_cal).regression_mProphet, mProphet_methods{numMethod})
                            if ~isempty(eval(['pepLibrarySRMCal(idx_lib_cal).regression_mProphet.' mProphet_methods{numMethod}]))
                                ...
                                if isfield(pepLibrarySRM(i).ratio_mProphet, mProphet_methods{numMethod}) && ~isempty(idxs_cc_dataFiles) && eval(['(nanmedian([pepLibrarySRM(i).ratio_mProphet(molarities_files(idxs_cc_dataFiles,2)).' mProphet_methods{numMethod}  ']).^(-1))~=0']) && ~isinf(eval(['(nanmedian([pepLibrarySRM(i).ratio_mProphet(molarities_files(idxs_cc_dataFiles,2)).' mProphet_methods{numMethod}  ']).^(-1))']))
